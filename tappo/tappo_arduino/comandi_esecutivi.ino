...
void leggi_fine_corsa(int buffer[4]){ // legge lo stato dei quattro fine corsa
  buffer[0] = 0;                      // e li scrive nel buffer dato
  buffer[1] = 1;
  buffer[2] = 0;
  buffer[3] = 0;
}
...
bool apri_petalo(int n_petalo){       // Inizia moto in senso di apertura del petalo.
  return true;                        // riporta: true in caso di successo, false altrimenti
}
...
bool stop_moto(int n_petalo){         // interrompe il moto
  return true;                        // riporta: true in caso di successo, false altrimenti
}
