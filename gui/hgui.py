...
CALIB_DATA_NAME = "calib_data.json"    # Name of calibration data file
...
class HomerGUI(tk.Frame):                                   # pylint: disable=R0901,R0902
    ...
    def __init__(self, master, basedir, simul=False):    # pylint: disable=R0915
        super().__init__(master, bg=BG_COL)
        self.arcsec_rate = 15
        self.donuts = None
        ...
        folderimg = wg.get_icon("folder", 24)
#       tk.Label(self, image=wg.get_icon("opc", 64),
#                bg=BG_COL).grid(row=18, rowspan=8, column=0, sticky=tk.W+tk.S)


        frame1 = tk.LabelFrame(self, text="Directories", fg=TX_COL, bg=BG_COL)

        tk.Label(frame1, text="Science folder ", fg=TX_COL,
                 bg=BG_COL).grid(row=0, column=0, sticky=tk.E)
        self.sci_dir_f = tk.Label(frame1, width=50, fg=TX_COL, bg='#5f5e58', anchor=tk.W)
        self.sci_dir_f.grid(row=0, column=1, columnspan=10)
        self.sci_dir_tt = wg.ToolTip(self.sci_dir_f, text="")
        tk.Button(frame1, image=folderimg, command=self.set_sci_folder,
                  bg=BG_COL).grid(row=0, column=11)
        self.realcalibnew = tk.StringVar()
        tk.Label(frame1, text="Calib. data file ", fg=TX_COL,
                 bg=BG_COL).grid(row=1, column=0, sticky=tk.E)
        self.sci_calib_f = tk.Label(frame1, anchor=tk.W, width=50, fg=TX_COL, bg='#5f5e58')
        self.sci_calib_f.grid(row=1, column=1, columnspan=10)
        self.sci_calib_tt = wg.ToolTip(self.sci_calib_f, text="")
#       tk.Button(frame1, image=folderimg, command=self.browse_calib_file,
#                 bg=BG_COL).grid(row=1, column=11)

        tk.Label(frame1, text="Use aux camera? ", fg=TX_COL,
                 bg=BG_COL).grid(row=2, column=0, sticky=tk.E)
        tk.Label(frame1, text="   Aux camera folder ", fg=TX_COL,
                 bg=BG_COL).grid(row=3, column=0, sticky=tk.E)
        self.use_aux = tk.IntVar()
        tk.Radiobutton(frame1, text="No  ", fg=TX_COL, bg=BG_COL,
                       selectcolor='green', variable=self.use_aux, value=0,
                       command=self.disable_aux).grid(row=2, column=1, sticky=tk.W)
        tk.Radiobutton(frame1, text="Yes", fg=TX_COL, bg=BG_COL,
                       selectcolor='green', variable=self.use_aux, value=1,
                       command=self.enable_aux).grid(row=2, column=2, sticky=tk.W)
        self.aux_dir_f = tk.Label(frame1, width=50, fg=TX_COL,
        ...
        self.use_aux_b = tk.Button(frame1, image=folderimg, state=tk.DISABLED,
                                   command=self.set_aux_folder, bg=BG_COL)
        self.use_aux_b.grid(row=3, column=11)
        tk.Label(frame1, text="Calib. data file ", fg=TX_COL,
                 bg=BG_COL).grid(row=4, column=0, sticky='e')
        self.aux_calib_f = tk.Label(frame1, state=tk.DISABLED,
                                    width=50, fg=TX_COL, bg='#5f5e58', anchor=tk.W)
        self.aux_calib_f.grid(row=4, column=1, columnspan=10)
        self.aux_calib_tt = wg.ToolTip(self.aux_calib_f, text="")
        frame1.grid(row=0, column=0, columnspan=2)

        frame21 = tk.Frame(self, bg=BG_COL)
        tk.Label(frame21, text=' ', bg=BG_COL, font='helvetica 9').pack(expand=1, fill=tk.X)
        frame211 = tk.LabelFrame(frame21, text="Parameters", fg=TX_COL, bg=BG_COL)

#       tk.Label(frame211, text="    ", bg=BG_COL).grid(row=1, column=0)
        tk.Label(frame211, text="   Aux camera ntiles ", fg=TX_COL,
                 bg=BG_COL).grid(row=1, column=1, sticky=tk.E)
        self.tiles_e = tk.Entry(frame211, width=6, fg=TX_COL, bg='#5f5e58')
        self.tiles_e.insert(0, "32")
        self.tiles_e.grid(row=1, column=2)
        tk.Label(frame211, text="  Sid. tracking freq. ", fg=TX_COL,
                 bg=BG_COL).grid(row=2, column=1, sticky='e')
        self.rate_e = tk.Entry(frame211, width=6, fg=TX_COL, bg='#5f5e58')
        self.rate_e.insert(0, "59.2")
        self.rate_e.grid(row=2, column=2)
        frame211.pack(anchor='w')
        tk.Label(frame21, text=' ', bg=BG_COL, font='helvetica 9').pack(expand=1, fill=tk.X)
        frame212 = tk.LabelFrame(frame21, text="Calibration", fg=TX_COL, bg=BG_COL)
        tk.Label(frame212, text="    ", bg=BG_COL).grid(row=1, column=1)

        tk.Label(frame212, text="Calib. mode: ", fg=TX_COL,
                 bg=BG_COL, pady=0).grid(row=1, column=2, sticky=tk.E)
        self.solve_mode = LOCAL_SOLVE
        self.solve_b = tk.Button(frame212, text=self.solve_mode, command=self.chg_mode,
                                 width=6, fg=TX_COL, bg=BG_COL, pady=0)
        self.solve_b.grid(row=1, column=3)
        tk.Button(frame212, text="Sci. image ", command=lambda: self.calibrate("sci"), width=10,
                  fg=TX_COL, bg=BG_COL, pady=0).grid(row=2, column=2, sticky=tk.W)
        self.sci_cal_stop = tk.Button(frame212, text="Stop", state=tk.DISABLED,
                                      command=self.cal_stop, width=6,
                                      fg=TX_COL, bg=BG_COL, pady=0)
        self.sci_cal_stop.grid(row=2, column=3, sticky=tk.W)
        self.aux_cal_start = tk.Button(frame212, text="Aux. image", state=tk.DISABLED,
                                       command=lambda: self.calibrate("aux"), width=10,
                                       fg=TX_COL, bg=BG_COL, pady=0)
        self.aux_cal_start.grid(row=3, column=2, sticky=tk.W)
        self.aux_cal_stop = tk.Button(frame212, text="Stop", state=tk.DISABLED,
                                      command=self.cal_stop, width=6,
                                      fg=TX_COL, bg=BG_COL, pady=0)
        self.aux_cal_stop.grid(row=3, column=3, sticky=tk.W)
        frame212.pack(expand=1, fill=tk.X)
#       tk.Button(self, text="Clear text", command=self.clear_text_box, height=2, width=10,
#                 fg=TX_COL, bg=BG_COL).grid(row=21, column=12, columnspan=2)
        tk.Label(frame21, text=' ', bg=BG_COL, font='helvetica 9').pack(expand=1, fill=tk.X)
        ...
        fig = Figure(figsize=(2.20, 2.20), dpi=100, facecolor=BG_COL)
        ...
        self.text_box = tk.Text(self, state=tk.DISABLED, fg=TX_COL, bg='#5f5e58')
