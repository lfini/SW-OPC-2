...
# Le funzioni sono implementate direttamente per diminuire il numero
...
import time
from math import sin, cos, sqrt, asin, atan2, pi, fmod, modf
...
class OPC:                         # pylint: disable=R0903
...
def float2ums(value, module=0, precision=0, as_string=''):
    "Converte valore float in sessagesimale (sign, units, minutes, secs[.frac])"
    if module:
        value %= module
    sign = 1 if value >= 0.0 else -1
    value = abs(value)
    rest, units = modf(value)
    rest, mins = modf(rest*60)
    units = int(units)
    mins = int(mins)
    if precision == 0:
        secs = int(round(rest*60))
    else:
        secs = round(rest*60, precision)
    if secs >= 60:
        secs -= 60
        mins += 1
        if mins >= 60:
    ...
    if as_string:
        ...
        if precision == 0:
            ...
            isecs = int(secs)
            rest = int((secs-isecs)*precision*10)
            secs = f'{isecs:02d}.{rest}'
        if as_string == 'h':
            return f'{sign}{units}h{mins:02d}m{secs}s'
        if as_string == 'd':
            return f'{sign}{units}°{mins:02d}\'{secs}"'
        return f'{sign}{units}:{mins:02d}:{secs}'
    return (sign, units, mins, secs)

def float2um(value, module=0, as_string=':'):
    "Converte valore float in sessagesimale (sign, units, minutes)"
    if module:
        value %= module
    sign = 1 if value >= 0.0 else -1
    value = abs(value)
    ...
    if as_string:
        sign = '+' if sign == 1 else '-'
        if as_string == 'h':
            return f'{sign}{units}h{mins:02d}m'
        if as_string == 'd':
            return f'{sign}{units}°{mins:02d}\''
        return f'{sign}{units}:{mins:02d}'
    return (sign, units, mins, secs)
