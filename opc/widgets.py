...
class Controller(tk.Frame):              # pylint: disable=R0901,R0902
    ...
    def __init__(self, parent, value=0, width=5, lower=MIN, upper=MAX,          # pylint: disable=R0914
...
class MButton(tk.Button):              # pylint: disable=R0901
    ...
    def __init__(self, parent, name, shape, size, value=None, command=None, **kw):
        self.name = name
        self.status = 0
        self.shape = shape
        self.size = size
        ...
        super().__init__(parent, image=image0, **kw)
        ncommand = None if command is None else lambda x=name: command(x)
        super().__init__(parent, command=ncommand, **kw)
        if value:
            self.set(value)
...
class Field(tk.Label):              # pylint: disable=R0901
    ...
    def __init__(self, parent, bg="black", fg="lightgreen",
        ...
        super().__init__(parent, text=text, bg=bg, fg=fg, width=width,
...
class Number(Field):              # pylint: disable=R0901
    ...
    def __init__(self, parent, /, value=None, fmt="%d", invalid='', **kw):
        super().__init__(parent, **kw)
        self.fmt = fmt
        self.invalid = invalid
        self.value = None
...
class Coord(Field):              # pylint: disable=R0901
    ...
    def __init__(self, parent, value=None, mode='°', precision='s', invalid='', **kw):
        '''
        Parametri
        ---------
        parent : widget
...
"""
    def __init__(self, parent, title=None, position=None, **kw):
        super().__init__(parent, **kw)
        if title:
            self.title(title)
...
class MessageText(tk.Text):              #pylint: disable=R0901
    'display di messaggi'
    def __init__(self, parent, msg, font=None, fg=None, bg=None):
        super().__init__(parent)
        ...
        self.body = tk.Text(self, height=n_lines, width=n_chars,
                            ...
                            padx=20, pady=5, border=3, relief=tk.RIDGE)
        self.body.insert(tk.END, msg)
        self.body.pack()

class Message(MyToplevel):
    "Display di messaggio (in Toplevel)"
    def __init__(self, parent, msg, title=None, position=None,
                 font=None, fg=None, bg=None, button="Chiudi"):
        super().__init__(parent, title=title, position=position)
...
class InfoMsg(Message):              # pylint: disable=R0901
    ...
    def __init__(self, parent, msg, title="Info", position=(30, 30), **kwd):
        super().__init__(parent, msg, title=title, position=position, **kwd)
        self.config(bg=INFO_CLR)

class WarningMsg(Message):              # pylint: disable=R0901
    "Display di messaggio warning"
    def __init__(self, parent, msg, title="Warning", position=(30, 30), **kwd):
        super().__init__(parent, msg, title=title, position=position, **kwd)
        self.config(bg=WARNING_CLR)

class ErrorMsg(Message):               # pylint: disable=R0901
    "Display di messaggio errore"
    def __init__(self, parent, msg, title="Error", position=(30, 30), **kwd):
        super().__init__(parent, msg, title=title, position=position, **kwd)
        self.config(bg=ERROR_CLR)

class YesNo(Message):              # pylint: disable=R0901
    "Display di messaggio con scelta opzioni Si/No"
    def __init__(self, parent, msg, position=(30, 30), **kwd):
        kwd["button"] = ""
        super().__init__(parent, msg, position=position, **kwd)
...
class SelectionMsg(Message):              # pylint: disable=R0901
    ...
    def __init__(self, parent, msg, choices=None, **kwd):
        kwd["button"] = ""
        super().__init__(parent, msg, **kwd)
        bot_frame = tk.Frame(self)
...
class HSpacer(tk.Label):              # pylint: disable=R0901
    ...
    def __init__(self, parent, nspaces=0, **kw):
        if nspaces:
            spaces = " "*nspaces
            super().__init__(parent, text=spaces, **kw)
...
class TestWidget(tk.Frame):
    ...
    def __init__(self, parent, wdg, *args, **kargs):
        super().__init__(parent)
